...
function add(message, sender, sp) {
  ...
  if (sender == myId || sender == myName) e.classList.add('self');
...
function addAudio(data, sender, sp, transcription) {
  let e = document.createElement('p');
  let aud = document.createElement('audio');
  aud.controls = true;
  aud.src = data;
  e.innerHTML = '<span>' + sender + (sp != null ? sp : '') + ':</span>';
  e.appendChild(aud);
  e.classList.add('message');
  ...
  messages.appendChild(e);
  chat += sender + ': ' + transcription + '\n';
  if (transcription != null && transcription.trim() != '') {
    add(transcription, sender, sp);
  }
}
...
function awardBadge() {
  let uid = window.prompt('enter the username you want to award the badge to');
    ...
    if (recording) {
      ...
      if (recorder == null) {
        ...
        recorder = new MediaRecorder(stream, {
          mimeType: 'audio/webm',
          audioBitsPerSecond: 705600 // 44100 (hz) * 16 (sampwidth 2) * 1 (channels)
        });
        ...
        recorder.onstop = event => {
          ...
          audioBlob.arrayBuffer().then(buffer => {
            ...
            sendAudio(`data:audio/webm;base64,${base64data}`)
          });
...
websocket.addEventListener("message", ({ data }) => {
  ...
  switch (event.type) {
    case 'message':
      sender = "senderreal" in event ? event.senderreal : event.sender;
      senderPlus = '';
      if ("points" in event) {
      ...
      break;
    case 'badge':
      add(`you have been awarded a kindness badge by ${"senderreal" in event ? event.senderreal : event.sender}! their reason: ${event.data}`, 'console', '')
    case 'resetSpoken':
    ...
    case 'audio':
      ...
      addAudio(event.data, sender, senderPlus, event.transcription);
      break;
    case 'close':
    ...
    case 'init':
      ...
      myId = event.sender;
      myName = event.senderreal;
      let e = document.createElement('p');
      e.innerText = 'Your user ID: ' + myId;
      actions.appendChild(e);
      
      e = document.createElement('p');
      e.innerText = 'Your username: ' + myName;
      actions.appendChild(e);

      e = document.createElement('p');
      e.innerText = 'Join Code: ' + event.data;
      actions.appendChild(e);

      e = document.createElement('a');
      e.href = `javascript:copy(\"${event.data}\");`;
      e.innerHTML = "<span class=\"icon link\"></span>Copy Join Code";
      e.target = "";
      actions.appendChild(e);
      ...
      e.innerHTML = "<span class=\"icon logout\"></span>Leave";
      e.target = "";
      actions.appendChild(e);
      break;
    case 'error':
...
websocket.addEventListener("open", () => {
  ...
  if (JSON.parse(localStorage.getItem('196532ac-ae33-4e85-952a-e3a969fa70bc-collablens')).currentAccount != event.username) {
